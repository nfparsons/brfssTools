...
### Stage B-cleanup: Drop the demographics/templates code (~0.5 session, parallel with B)

- Delete `R/demographics_setup.R`
- Delete `inst/extdata/transformations/templates/` (all 18 files)
- Remove `brfss_setup_demographic()`, `brfss_list_demographic_templates()`,
  `brfss_demographic_status()` exports
- Remove all references in NAMESPACE, NEWS, TODO, package help

Deliverable: smaller, less confused package surface.
...
| 10 | Drop the Demographics tab and all 18 templates entirely; demographics are not architecturally special | 2026-05-01 |
