...
cw_load <- function(path = NULL) {
  ...
  # Schema check. v0.2.0 schema:
  required_cw <- c("concept_id", "year", "state_var", "is_calculated",
                   "calculation_yaml", "domain", "subdomain",
                   "unverified", "notes")
  missing_cw <- setdiff(required_cw, names(bundle$crosswalk))

  # If we see v0.1.0-only columns, the user needs to migrate.
  ...
  if (length(v01_only) > 0L && length(missing_cw) > 0L) {
    stop(
      ...
      ").\nRun brfss_migrate_crosswalk_to_v2() to update the schema.",
      call. = FALSE
    )
  }
...
# Stub kept for backward compat: nothing to do under v0.2.0
.cw_ensure_concept_id <- function(bundle) bundle
...
cw_save <- function(bundle, path = NULL) {
  ...
  # In v0.2.0, only crosswalk.csv is mutable user data.
  if (!is.null(bundle$crosswalk)) {
    readr::write_csv(bundle$crosswalk, file.path(path, "crosswalk.csv"),
                     na = "")
  }
